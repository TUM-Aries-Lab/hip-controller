...
# The preprocessor takes a BasicConfig; its PreprocessorConfig is derived from
# that config's `frequency`, so the sample-rate dependent filters follow the
# controller rate rather than being built by hand.
preprocessor = SensorPreprocessor(basic_config=BasicConfig())
